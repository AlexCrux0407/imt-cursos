...
$page_title = 'Docente – Lecciones del Subtema';
...
$stmt = $conn->prepare("
    ...
    ORDER BY orden ASC
");
$stmt->execute([':subtema_id' => $subtema_id]);
...
<div class="contenido">
    <div class="form-container-head" style="background: linear-gradient(135deg, #3498db, #3498db); color: white;">
        <div class="div-fila-alt-start">
            <div>
                <h1 style="font-size: 2rem; margin-bottom: 10px;">Lecciones del Subtema</h1>
                <p style="opacity: 0.9;"><?= htmlspecialchars($subtema['titulo']) ?> - <?= htmlspecialchars($subtema['curso_titulo']) ?></p>
            </div>
    ...
    <div class="form-container-body" style="margin-bottom: 20px;">
        <div class="div-fila-alt-start" style="gap: 10px;">
            ...
            <span style="color: #7f8c8d;"> > </span>
            <a href="<?= BASE_URL ?>/docente/temas_modulo.php?id=<?= $modulo_id ?>&curso_id=<?= $curso_id ?>" style="color: #7f8c8d; text-decoration: none;">Temas</a>
            <span style="color: #7f8c8d;"> > </span>
            <a href="<?= BASE_URL ?>/docente/subtemas_tema.php?id=<?= $tema_id ?>&modulo_id=<?= $modulo_id ?>&curso_id=<?= $curso_id ?>" style="color: #7f8c8d; text-decoration: none;">Subtemas</a>
            <span style="color: #7f8c8d;"> > </span>
    ...
    <div class="form-container-body" style="margin-bottom: 20px;">
        <div class="div-fila" style="gap: 20px; align-items: center;">
            <div style="flex: 1;">
                <h3 style="color: #2c3e50; margin-bottom: 10px;"><?= htmlspecialchars($subtema['titulo']) ?></h3>
                <p style="color: #7f8c8d; margin-bottom: 5px;"><?= htmlspecialchars($subtema['tema_titulo']) ?> · <?= htmlspecialchars($subtema['modulo_titulo']) ?></p>
                <span style="color: #3498db; font-weight: 500;"><?= count($lecciones) ?> lecciones</span>
            </div>
            <div>
                <button onclick="window.history.back()" class="btn-volver">← Volver a Subtemas</button>
            </div>
        </div>
    </div>
    ...
    <div class="form-container-body">
        ...
        <?php if (empty($lecciones)): ?>
            <div style="text-align: center; padding: 40px; color: #7f8c8d;">
                ...
                <p>Comienza agregando la primera lección a este subtema</p>
                <button onclick="mostrarFormularioNuevaLeccion()" 
...
function editarLeccion(id) {
    window.location.href = `<?= BASE_URL ?>/docente/editar_leccion.php?id=${id}&modulo_id=<?= $modulo_id ?>&curso_id=<?= $curso_id ?>&tema_id=<?= $tema_id ?>&subtema_id=<?= $subtema_id ?>`;
}

function eliminarLeccion(id) {
    if (confirm('¿Estás seguro de que deseas eliminar esta lección?')) {
        window.location.href = `<?= BASE_URL ?>/docente/eliminar_leccion.php?id=${id}&modulo_id=<?= $modulo_id ?>&curso_id=<?= $curso_id ?>&tema_id=<?= $tema_id ?>&subtema_id=<?= $subtema_id ?>`;
    }
}
