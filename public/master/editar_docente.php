...
$page_title = 'Master – Editar Docente';
...
$stmt = $conn->prepare("SELECT * FROM usuarios WHERE id = :id AND role = 'docente' LIMIT 1");
$stmt->execute([':id' => $id]);
$usuario = $stmt->fetch();
if (!$usuario) {
    ...
    if (empty($errors)) {
        try {
            if (!empty($password)) {
                ...
                $stmt = $conn->prepare("UPDATE usuarios SET nombre = :nombre, email = :email, estado = :estado, password = :password, updated_at = NOW() WHERE id = :id AND role = 'docente'");
                $stmt->execute([
                    ':nombre' => $nombre,
                    ':email' => $email,
                    ':estado' => $estado,
                ...
                $stmt = $conn->prepare("UPDATE usuarios SET nombre = :nombre, email = :email, estado = :estado, updated_at = NOW() WHERE id = :id AND role = 'docente'");
                $stmt->execute([
                    ':nombre' => $nombre,
                    ':email' => $email,
                    ':estado' => $estado,
            ...
            header('Location: ' . BASE_URL . '/master/editar_docente.php?id=' . $id . '&success=perfil_actualizado');
            exit;
        } catch (Throwable $e) {
            header('Location: ' . BASE_URL . '/master/editar_docente.php?id=' . $id . '&error=database_error&detalle=' . urlencode($e->getMessage()));
            exit;
        }
    } else {
        header('Location: ' . BASE_URL . '/master/editar_docente.php?id=' . $id . '&error=validacion&detalle=' . urlencode(implode('\n', $errors)));
        exit;
    }
}
...
<div class="contenido">
    <div class="form-container-head" style="background: linear-gradient(135deg, #3498db, #3498db); color: white; text-align: center;">
        <h1 style="margin: 0; font-size: 2rem; font-weight: 600;">Editar Docente</h1>
        <p style="margin: 5px 0 0 0; opacity: 0.9;">Actualiza la información del perfil del docente</p>
    </div>
            ...
            <div class="div-fila" style="gap: 16px;">
                ...
                <div style="flex: 1;">
                    ...
                    <div style="display: flex; gap: 8px; align-items: center;">
                        <input type="password" id="passwordEditarDocente" name="password" placeholder="Dejar en blanco para no cambiar" 
                               style="flex: 1; padding: 10px; border: 2px solid #e8ecef; border-radius: 8px; font-size: 1rem;">
                        <button type="button" onclick="togglePasswordVisibility('passwordEditarDocente', this)"
                        ...
                        <button type="button" onclick="generarPasswordTemporal('passwordEditarDocente', this)"
