    ...
    if (empty($errors)) {
        try {
            if (!empty($password)) {
                ...
                $stmt = $conn->prepare("UPDATE usuarios SET nombre = :nombre, email = :email, usuario = :usuario, estado = :estado, password = :password, updated_at = NOW() WHERE id = :id AND role = 'docente'");
                $stmt->execute([
                    ':nombre' => $nombre,
                    ':email' => $email,
                ...
                $stmt = $conn->prepare("UPDATE usuarios SET nombre = :nombre, email = :email, usuario = :usuario, estado = :estado, updated_at = NOW() WHERE id = :id AND role = 'docente'");
                $stmt->execute([
                    ':nombre' => $nombre,
                    ':email' => $email,
                    ':usuario' => $usuario_name,
