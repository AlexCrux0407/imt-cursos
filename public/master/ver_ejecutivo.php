...
$page_title = 'Master – Ver Ejecutivo';
...
// Estadísticas básicas del ejecutivo
$stmt = $conn->prepare("
    SELECT 
    ...
    WHERE c.asignado_a = :ejecutivo_id
");
$stmt->execute([':ejecutivo_id' => $id]);
$stats = $stmt->fetch();
...
<div class="contenido">
    <div class="form-container-head" style="background: linear-gradient(135deg, #3498db, #3498db); color: white; text-align: center;">
        <h2 style="margin: 0; font-size: 1.8rem; font-weight: 600;">Detalle del Ejecutivo</h2>
    ...
    <div class="form-container-body">
        ...
        <div class="div-fila" style="gap: 20px;">
            <div style="flex: 1; background: #f8f9fa; padding: 16px; border-radius: 8px;">
                ...
                <div style="color: #34495e;"><?= htmlspecialchars(format_nombre($ejecutivo['nombre'])) ?></div>
            </div>
            <div style="flex: 1; background: #f8f9fa; padding: 16px; border-radius: 8px;">
        ...
        <div class="div-fila" style="gap: 20px; margin-top: 15px;">
            <div style="flex: 1; background: #f8f9fa; padding: 16px; border-radius: 8px;">
                ...
                <div>
                    <span style="padding: 6px 12px; border-radius: 20px; font-size: 0.9rem; font-weight: 500; background: <?= $ejecutivo['estado'] === 'activo' ? '#d4edda' : '#f8d7da' ?>; color: <?= $ejecutivo['estado'] === 'activo' ? '#155724' : '#721c24' ?>;">
            ...
            <div style="flex: 1; background: #f8f9fa; padding: 16px; border-radius: 8px;">
                ...
                <div style="color: #34495e;"><?= date('d/m/Y', strtotime($ejecutivo['created_at'])) ?></div>
            </div>
        </div>
    </div>
