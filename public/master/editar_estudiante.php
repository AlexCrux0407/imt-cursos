...
$page_title = 'Master – Editar Estudiante';
...
$stmt = $conn->prepare("SELECT * FROM usuarios WHERE id = :id AND role = 'estudiante' LIMIT 1");
$stmt->execute([':id' => $id]);
$usuario = $stmt->fetch();
if (!$usuario) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            header('Location: ' . BASE_URL . '/master/editar_estudiante.php?id=' . $id . '&error=database_error&detalle=' . urlencode($e->getMessage()));
            exit;
        }
    } else {
        header('Location: ' . BASE_URL . '/master/editar_estudiante.php?id=' . $id . '&error=validacion&detalle=' . urlencode(implode('\n', $errors)));
        exit;
    }
}
...
<div class="contenido">
    <div class="form-container-head" style="background: linear-gradient(135deg, #3498db, #3498db); color: white; text-align: center;">
        <h1 style="margin: 0; font-size: 2rem; font-weight: 600;">Editar Estudiante</h1>
        <p style="margin: 5px 0 0 0; opacity: 0.9;">Actualiza la información del perfil del estudiante</p>
    </div>
            ...
            <div class="div-fila" style="gap: 16px;">
                <div style="flex: 1;">
                    ...
                    <div style="display: flex; gap: 8px; align-items: center;">
                        <input type="password" id="passwordEditarEstudiante" name="password" placeholder="Dejar en blanco para no cambiar" 
                               style="flex: 1; padding: 10px; border: 2px solid #e8ecef; border-radius: 8px; font-size: 1rem;">
                        <button type="button" onclick="togglePasswordVisibility('passwordEditarEstudiante', this)"
                        ...
                        <button type="button" onclick="generarPasswordTemporal('passwordEditarEstudiante', this)"
